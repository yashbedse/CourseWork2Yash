<div class="dc-consultation">
    <div class="dc-searchresult-head">
        <div class="dc-title"><h4>“{{ !empty($user->profile->gender_title) ? $user->profile->gender_title : '' }} {{ Helper::getUserName($user->id) }}” {{ trans('lang.answers') }}</h4></div>
    </div>
                ...
                @if (!empty($speciality))
                    <div class="dc-consultation-details">
                        ...
                        <div class="dc-consultation-title">
                            <h5><a href="{{ route('getForumAnswers', ['slug' => clean($forum->slug)]) }}">{{ html_entity_decode(clean($forum->question_title)) }}</a><em>{{ Carbon\Carbon::parse($forum->created_at)->format('M d, Y') }}</em></h5>
