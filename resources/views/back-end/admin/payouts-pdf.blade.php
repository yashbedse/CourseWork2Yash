            ...
            <table style="width:96%; margin:80px auto 0;">
                <tr style="text-align:left;">
                    <td width="70%">
                        ...
                        <span style="font-size:16px;line-height: 20px;display: block; color: #3d4461;">{{ trans('lang.payout_history_note') }} {{{ clean($year) }}},{{{ clean($month) }}}</span>
                    </td>
                </tr>
            </table>
        </div>
            ...
            <tbody>
                @foreach ($payouts as $key => $payout)
                    <tr>
                        ...
                        <td style="padding: 15px 20px;border-top: 1px solid #e2e2e2; font-size:14px;">{{ Helper::currencyList($payout->currency)['symbol'] }}{{{ clean($payout->amount) }}}</td>
                        ...
                        <td style="padding: 15px 20px;border-top: 1px solid #e2e2e2;word-wrap:break-word; font-size:14px;">
                            @if (!empty(($payout->payout_detail)))
                                @php $detail = Helper::getUnserializeData($payout->payout_detail); @endphp
                                @if ($detail['type'] == 'bacs')
                                    {{ trans('lang.bank_account_name') }}: {{ html_entity_decode(clean($detail['bank_account_name'])) }} 
                                    {{ trans('lang.bank_account_number') }}: {{ clean($detail['bank_account_number']) }} 
                                    {{ trans('lang.bank_name') }}: {{html_entity_decode(clean($detail['bank_name'])) }} 
                                    {{ trans('lang.bank_routing_number') }}: {{ html_entity_decode(clean($detail['bank_routing_number'])) }} 
                                    {{ trans('lang.bank_iban') }}: {{ html_entity_decode(clean($detail['bank_iban'])) }} 
                                @else
                                    {{ trans('lang.email') }}: {{ html_entity_decode(clean($detail['paypal_email'])) }} 
                                @endif
                            @elseif (!empty($payout->paypal_id))
                                {{{$payout->paypal_id}}}
                            @endif
                        </td>
                        <td style="padding: 15px 20px;border-top: 1px solid #e2e2e2; font-size:14px;">{{{ clean($payout->status) }}}</td>
                    </tr>
                    @php $counter++; @endphp
                @endforeach
            </tbody>
