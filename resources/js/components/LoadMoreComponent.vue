<template>
    <div>
        <li v-for="(showposts, index) in showposts" :key="index" v-if = "items[index]">
            <span v-if="items[index].degree_title">{{ items[index].degree_title }}
                ...
                <div class="d-block text-left">
                    <div class="wt-formtheme wt-formfeedback">
                        <ul class="dc-expandedu">
                            <li>
                                <span><strong>{{ trans('lang.degree_title') }}:</strong><em> {{ items[index].degree_title }}</em></span>
                            </li>
                            <li>
        ...
        <a href="javascript:void(0);" @click="showposts += 2" v-if="showposts < items.length">
            {{ trans('lang.more') }}
        </a>
    </div>
</template>
<script>
    export default{
        props: ['no_of_post', 'modal_ref', 'url', 'doctor_id', 'modal_title'],
        data() {
            return {
                items: [],
